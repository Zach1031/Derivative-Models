...
float Simulation::runSimulation(Option *O, int simulations) {
    ...
    auto simulate=[](int sims, int N, float initPrice, float nudt, float sigsdt, float &sumCT, float K) {
        std::random_device seed;
        std::default_random_engine generator(seed());
        std::normal_distribution<double> distribution(0,1);
        ...
        for (int i = 0; i < sims; i++) {
            ...
            sumCT += std::max(exp(logPrice) - K, 0.0f);
        }
    };
    ...
    for (int i = 0; i < THREADS; i++) {
        threads.push_back(std::thread(simulate, simulations / THREADS, N, initLogPrice, nudt, sigsdt, std::ref(sumCT), O->getK()));
    }
